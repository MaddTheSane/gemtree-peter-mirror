...
extern	bool	Wsainit2;				// knihovna WSA 2.2 úspěšně inicializována
...
extern WSAOVERLAPPED UDPSendOver;		// overlapped struktura vysílače
extern WSABUF		UDPSendBuf;			// odesílací buffer UDP
extern bool			UDPSending;			// probíhá vysílání UDP

extern SOCKET		UDPRecSocket;		// UDP přijímací soket (INVALID_SOCKET = není)
extern SOCKADDR_IN	UDPRecAddr;			// přijímací adresa UDP
extern SOCKADDR_IN	UDPRecRecAddr;		// přijatá adresa UDP
extern WSAOVERLAPPED UDPRecOver;		// overlapped struktura přijímače
extern WSABUF		UDPRecBuf;			// přijímací buffer UDP
extern bool			UDPReceiving;		// probíhá overlapped příjem UDP
extern int			UDPRecData;			// velikost dat v přijímacím bufferu
...
bool OpenSendUDP();						// otevření vysílacího soketu UDP (při chybě vrací FALSE)
void CloseSendUDP();					// zavření vysílacího soketu UDP
bool OpenRecUDP();						// otevření přijímacího soketu UDP (při chybě vrací FALSE)
void CloseRecUDP();						// zavření přijímacího soketu UDP
void SendUDP(const void* data, int len);// odeslání UDP paketu
#ifdef UDP_OVER
void StartRecUDP();						// start příjmu UDP paketu
#endif
