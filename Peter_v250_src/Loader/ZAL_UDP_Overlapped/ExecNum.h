...
#define FILETIMEKONV 10000000		// pøevod na souborový èas
...
double FLn();						// pøirozený logaritmus
double FExp();						// pøirozený exponent
...
double FGetPicHeight();				// zjištìní výšky obrázku
...
double FTimeUtc();					// pøevod na svìtový èas
...
double FGetTimeWeek();				// naètení dne v týdnu z èasu
...
double FTabsNew();					// vytvoøení záložek
...
double FGetWindowHInt();			// vnitøní výška okna
double FGetFontHeight();			// výška fontù
...
double FD3DCone();					// vytvoøení kužele
double FD3DCone2();					// vytvoøení komolého kužele
...
double FGetD3DAmbiRef();			// zjištìní odrazu rozptýleného svìtla
...
double FGetD3DSplits();				// zjištìní složitosti objektù
...
double FD3DElapsed0();				// uplynulý pøesný èas 
double FD3DElapsed();				// uplynulý pøesný èas 
...
double FGetD3DBias();				// zjištìní zjemnìní vzdálených textur
double FGetD3DMipMaps();			// poèet úrovní mipmaps

double FPolarRadius();				// výpoèet vzdálenosti bodu
double FPolarAngle();				// výpoèet úhlu bodu
double FPreciseTimer();				// pøesný èítaè èasu
...
// tabulky pro tvorbu zdi 3D, používají se i pøi zobrazení 2D objektu v OpenGL
