...
WSAHTONS pWSAhtons = NULL;				// funkce "htons" (převede WORD číslo portu - na PC prohodí bajty)
...
WSASENDTO pWSASendTo = NULL;			// funkce "sendto"
WSARECVFROM pWSARecvFrom = NULL;		// funkce "recvfrom"
WSAINETADDR pWSAInetAddr = NULL;		// funkce "inet_addr"
WSASELECT pWSASelect = NULL;			// funkce "select"
...
bool	Wsainit2 = false;			// rozšíření pro UDP úspěšně inicializováno
...
char*		UDPRecBuf = NULL;		// přijímací buffer UDP
int			UDPRecBufSize = 512;	// velikost přijímacího bufferu UDP
...
void WSAInitialize()
{
	...
	if (!Wsaload)
	{
		...
		// načtení knihovny WSA verze 1.1
		...
		if (WSALibrary != NULL)
		{
				...
				(pWSAInetNtoa != NULL))
			{
				...
				if (pWSAStartup(MAKEWORD(1, 1), &Wsadata) == 0)
				{
					...
					pWSASendTo = (WSASENDTO) ::GetProcAddress(WSALibrary, "sendto");
					pWSARecvFrom = (WSARECVFROM) ::GetProcAddress(WSALibrary, "recvfrom");
					pWSAInetAddr = (WSAINETADDR) ::GetProcAddress(WSALibrary, "inet_addr");
					pWSASelect = (WSASELECT) ::GetProcAddress(WSALibrary, "select");
...
/////////////////////////////////////////////////////////////////////////////
// otevření soketu UDP (při chybě vrací FALSE)
...
void SendUDP(const void* data, int len)
{
	if (WSAInitialize2())
	{
		if (OpenUDP())
		{
			fd_set fd;
			fd.fd_count = 1;
			fd.fd_array[0] = UDPSocket;

			struct timeval to;
			to.tv_sec = 0;
			to.tv_usec = 100000;

			int res = pWSASelect(UDPSocket+1, NULL, &fd, NULL, &to);
			if (res > 0)
				pWSASendTo(UDPSocket, (const char*)data, len, 0, (SOCKADDR*)&UDPSendAddr, sizeof(SOCKADDR_IN));
		}
	}
}
...
int RecUDP()
{
	if (WSAInitialize2())
	{
		if (OpenUDP())
		{
			...
			int res = pWSASelect(UDPSocket+1, &fd, NULL, NULL, &to);
			if (res > 0)
			{
				int fromlen = sizeof(SOCKADDR_IN);
				int n = pWSARecvFrom(UDPSocket, UDPRecBuf, UDPRecBufSize, 0,
							(SOCKADDR*)&UDPRecRecAddr, &fromlen);
				if (n < 0) n = 0;
				if (n > (int)UDPRecBufSize) n = UDPRecBufSize;
				return n;
			}
		}
	}
