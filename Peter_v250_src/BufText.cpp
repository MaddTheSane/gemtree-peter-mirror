...
CText::CText(LPCSTR text) // při chybě paměti bude řetězec prázdný
{
	...
	if (pData != NULL)
	{
		::MultiByteToWideChar(CodePage, 0, text, len, pData->Data, len);
	}
	else
	{
		attach(&EmptyTextData); 
...
CText::CText(LPCWSTR text) // při chybě paměti bude řetězec prázdný
{
	...
	if (pData != NULL)
	{
		::WideCharToMultiByte(CodePage, 0, text, len, pData->Data, len, NULL, NULL);
	}
	else
	{
		attach(&EmptyTextData); 
...
{
	::WideCharToMultiByte(CodePage, 0, pData->Data, pData->Length, buf, pData->Length, NULL, NULL);
}
...
{
	::MultiByteToWideChar(CodePage, 0, pData->Data, pData->Length, buf, pData->Length);
}
...
// středoevropská sada (1250)
const char* NormTabCE=
	"................................"
	"_...............0123456789......"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ...._"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ....."
	"C..F......S.STZZ..........S.STZZ"
	"...L.A...CS...RZ...L.U...AS.L.LZ"
	"RAAAALCCCEEEEIIDDNNOOOO.RUUUUYT."
	"RAAAALCCCEEEEIIDDNNOOOO.RUUUUYT.";

// ruská sada (1251)
const char* NormTabCY=
	"................................"
	"_...............0123456789......"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ...._"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ....."
	".G.G....C.L.YKHC..........L.YKHC"
	".UUJ.I..ECE...RI..IIR...ENE.JSSI"
	"ABVGDEZZIJKLMNOPRSTUFHCCSS.Y.EUA"
	"ABVGDEZZIJKLMNOPRSTUFHCCSS.Y.EUA";

// západoevropská sada (1252)
const char* NormTabWE=
	"................................"
	"_...............0123456789......"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ...._"
	".ABCDEFGHIJKLMNOPQRSTUVWXYZ....."
	"C..F......S.C.............S.C..Y"
	"..CL.Y...C....R...23.....1......"
	"AAAAAAACEEEEIIIIDNOOOOO.OUUUUY.."
	"AAAAAAACEEEEIIIIDNOOOOO.OUUUUY.Y";

void CText::NormAscii()
{
	if (!CopyWrite()) return;
	...
	{
		pData->Data[i] = normtab[(BYTE)(pData->Data[i])];
...
// kopie textu
	::MultiByteToWideChar(CodePage, 0, txt, len, data->Data, len);
...
// kopie textu
	::WideCharToMultiByte(CodePage, 0, txt, len, data->Data, len, NULL, NULL);
...
/////////////////////////////////////////////////////////////////////////////
// načtení textu z resource do dialogového prvku

void CText::LoadDialogText(const HWND wnd, int id)
{
	Load(id);
	SetDialogText(wnd, id);
}

/////////////////////////////////////////////////////////////////////////////
// načtení textu z resource do dialogového prvku s nastavením běžného fontu

void CText::LoadDialogTextFont(const HWND wnd, int id)
{
	Load(id);
	SetDialogTextFont(wnd, id);
}


/////////////////////////////////////////////////////////////////////////////
// načtení textu z resource (při chybě vrací FALSE, obsah prázdný text)

bool CText::Load(const int nID)
{
	const TEXTCONST* textconst = JazykInfo[Jazyk].TextConst;

	for (int i = 0; i < TextConstNum; i++)
	{
		if (textconst[i].TextID == nID)
		{
			return Set(textconst[i].Text);
		}
	}
...
void CText::KorigShort()
{
	if (ShortName)
	{
		NormAscii();					// normalizace na ASCII znaky
		int pos;
		while ((pos = Find(_T('_'))) >= 0)
		{
			Delete(pos, 1);
		}
		Delete(8);						// omezení na délku 8 znaků
	}
	else
	{
		FileName(0);
	...
	if (IsEmpty()) *this = _T('#');	// náhradní jméno
}
...
void _fastcall CText::SetWindowTextFont(const HWND wnd) const
{
	if (wnd != NULL)
	{
		::SetWindowText(wnd, pData->Data);
...
void _fastcall CText::SetDialogTextFont(const HWND wnd, int id) const
{
	HWND item = ::GetDlgItem(wnd, id);
	SetWindowTextFont(item);
}
...
// načtení textu do bufferu
	TCHAR buf[512];
	int len = (int)::GetCurrentDirectory(511, buf);
...
template <> bool CBuffer<CText>::UndoAddIns(int index)
{
	return Undo.AddTextIns(index);
}
...
int _fastcall CBufText::Add(TEXTDATA* data)
{
	CText text(data);
	int result = Add(text);
	return result;
...
int _fastcall CBufText::Add(const WCHAR* data)
{
	CText text(data);
	int result = Add(text);
	return result;
...
int _fastcall CBufText::Add(const char* data)
{
	CText text(data);
	int result = Add(text);
	return result;
...
////////////////////////////////////////////////////////////////////
// načtení textu z resource (vrací index položky, <0 = chyba)
...
template <> bool CBuffer<CMultiText>::UndoAddIns(int index)
{
	return Undo.AddTextIns(index);
}

// zjištění běžného textu
const CText& _fastcall CMultiText::MultiText() const
{
	if (m_Data[Jazyk].IsNotEmpty())
	{
		return m_Data[Jazyk];
	}
	
	if (m_Data[JazykDef].IsNotEmpty())
	{
		return m_Data[JazykDef];
	}

	if (m_Data[JAZYKENG].IsNotEmpty())
	{
		return m_Data[JAZYKENG];
	}

	for (int i = 0; i < JAZYKNUM; i++)
	{
		if ((i != JAZYK000) && (m_Data[i].IsNotEmpty()))
		{
			return m_Data[i];
		}
	}

	return m_Data[JAZYK000];
}

// načtení textu z resource
void _fastcall CMultiText::Load(const int nID)
{
	int oldjazyk = Jazyk;  

	for (Jazyk = 0; Jazyk < JAZYKNUM; Jazyk++)
	{
		m_Data[Jazyk].Load(nID);
	}
...
// nastavení čísla jména objektu (1, 2, ...)
// při chybě paměti vrátí FALSE, obsah bude nezměněn
bool _fastcall CMultiText::SetNumObj(const int num)
{
	BOOL ok = false;

	for (int lan = 0; lan < JAZYKNUM; lan++)
	{
		if (m_Data[lan].IsNotEmpty())
		{
			ok = true;
			if (!m_Data[lan].SetNumObj(num)) return false;
		}
	}

	if (!ok)
	{
		return m_Data[Jazyk].SetNumObj(num);
	}
	return true;
}

int _fastcall CBufMultiText::Load(const int nID)
{
// vytvoření nové položky
	int result = New();		// vytvoření nové položky
	if (result >= 0)
