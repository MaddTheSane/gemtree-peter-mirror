...
// importování obrázku z bufferu TRUECOLORALPHABETA (vrací FALSE=chyba pamìti)
	bool ImportTrueAlphaBeta(BYTE* buf, BOOL dith);

// zmìna velikosti obrázku (inter = interpolovat, dith = ditherovat)
// pøi chybì pamìti vrátí FALSE, obsah bude nezmìnìn
	bool Zoom(int width, int height, BOOL inter, BOOL dith);
...
// naètení obrázku ze souboru, comp=komprimovat (FALSE=chyba, obsah nezmìnìn)
	bool LoadFile(CText jmeno, BOOL comp);
...
class CBufPic : public CBuffer<CPicture>
{
...
// provádí záznam do UNDO bufferu
	inline int New() { return CBuffer<CPicture>::New(); } // prázdní obrázek ICONSIZE
	int _fastcall New(int width, int height);			// obrázek vymaže prùhlednou barvou

// pøidání položky (vrací index položky, <0 = chyba)
// provádí záznam do UNDO bufferu
	inline int _fastcall Add(const CPicture& data) { return CBuffer<CPicture>::Add(data); }
